...
async function uploadImpeachCommentFile_async({req}){
    ...
    if(false===originalDoc){
        return Promise.reject(controllerError.upload.notImpeachCreatorCantUploadFile)
    }
    // let =misc.objectDeepCopy(tmpResult)

    /************************************************/
    /*****************     特定检测      ************/
    /************************************************/
    //impeach的CURRENT_STATE必须为NEW或者EDITING。和impeach的检查点不同
/*    let impeach=await common_operation_model.findById_returnRecord_async({dbModel:e_dbModel.impeach,id:recordId})
    if(null!==impeach){*/
