...
async function getAllSendRecommend_async({req}){
    // ap.inf('getAllSendRecommend_async in')
    /********************************************************/
    /*************      define variant        ***************/
    /********************************************************/
    let tmpResult,condition,option
    // ap.inf('controller_setting.MAIN_HANDLED_COLL_NAME',controllerSetting)
    let collName=e_coll.SEND_RECOMMEND
...
// ap.inf('getRecord',getRecord)
    ...
    /********       保留所有字段         *******/
    /*********************************************/
    // let keepFields=[e_field.SEND_RECOMMEND.]
    /*if('unread'===type){
...
// ap.inf('after keep',getRecord[singleRecordIdx])
        ...
        populateFields={
            ...
            [e_field.SEND_RECOMMEND.RECEIVERS]:{
                'collName':e_coll.USER,
                'subPopulateFields':undefined,
            },
...
/*    if('read'===type){
    ...
    populateOpt.push(
        {
            path:e_field.SEND_RECOMMEND.RECEIVERS,
            // match:{},
    ...
    populateOpt.push(
        {
            path:e_field.SEND_RECOMMEND.SENDER,
            // match:{},
            // select:`{id:0, ${e_field.ARTICLE_ATTACHMENT.NAME}:1, ${e_field.ARTICLE_ATTACHMENT.HASH_NAME}:1}`,
            select:`${e_field.USER.NAME} `, //${e_field.ARTICLE_ATTACHMENT.HASH_NAME}是为了防止文件名冲突，导致文件覆盖，无需传递到前端
            // options:{limit:maxNumber.user_operation.maxRecommendPerPage},
...
// ap.inf('populateOpt',populateOpt)
    ...
    let result=await common_operation_model.find_returnRecords_async({dbModel:e_dbModel.send_recommend,condition:condition,options:options,populateOpt:populateOpt})
