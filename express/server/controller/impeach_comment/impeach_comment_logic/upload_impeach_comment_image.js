...
async function uploadImpeachCommentFile_async({req}){
    ...
    let recordId=req.body.values[e_part.RECORD_ID]  //impeachCommentId。容纳上传文件的主体
    ...
    tmpResult=await controllerChecker.ifCurrentUserTheOwnerOfCurrentRecord_yesReturnRecord_async({dbModel:e_dbModel[collName],recordId:recordId,ownerFieldName:e_field.IMPEACH_COMMENT.AUTHOR_ID,ownerFieldValue:userId,additionalCondition:undefined})
    if(false===tmpResult){
   ...
   /* if(e_uploadFileType.IMAGE===type){
    ...
    maxFileSize=uploadFileDefine.impeach_comment.image[e_uploadFileDefinitionFieldName.MAX_SIZE_IN_BYTE]
            ...
            case e_resourceProfileRange.IMAGE_PER_PERSON_FOR_WHOLE_IMPEACH:
                ...
                if(tmpResult.length>0){
                    for(let singleEle of tmpResult){
                        arr_impeach_and_comment_id.push(singleEle[e_field.IMPEACH_COMMENT.IMPEACH_ID])
                    }
                }
...
// console.log(`rexource check done ======================`)
    ...
    // if(e_uploadFileType.IMAGE===type){
        ...
        internalValue[e_field.IMPEACH_IMAGE.REFERENCE_COLL]=e_coll.IMPEACH_COMMENT
