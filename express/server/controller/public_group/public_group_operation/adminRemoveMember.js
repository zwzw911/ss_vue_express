...
async function adminRemoveMember_async({req}){
...
async function requestJoin_logic_async({req,expectedPart}){
    ...
    for(let singleMember of manipulateArrayValue[e_field.PUBLIC_GROUP.MEMBERS_ID][e_manipulateOperator.REMOVE]){
        ...
        for(let keyValues of manipulateArrayValue[e_field.PUBLIC_GROUP.WAIT_APPROVE_ID]){
            for(let singleEle of keyValues){
        ...
        //     }
        //     updateFieldsValue['$pullAll'][e_field.PUBLIC_GROUP.WAIT_APPROVE_ID].concat(manipulateArrayValue[e_field.PUBLIC_GROUP.WAIT_APPROVE_ID][e_manipulateOperator.ADD])
