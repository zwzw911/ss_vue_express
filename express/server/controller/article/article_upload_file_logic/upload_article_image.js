...
// console.log(`condition to check user =====>${JSON.stringify(condition)}`)
    ...
    let resourceProfileRangeToBeCheck=[e_resourceProfileRange.IMAGE_PER_ARTICLE,e_resourceProfileRange.WHOLE_RESOURCE_PER_PERSON_FOR_ALL_ARTICLE]
    ...
    for(let singleResourceProfileRecord of resourceResult){
        switch (singleResourceProfileRecord[e_field.RESOURCE_PROFILE.RANGE]){
            case e_resourceProfileRange.IMAGE_PER_ARTICLE:
                calcResult=await controllerHelper.calcExistResource_async({resourceProfileRange:e_resourceProfileRange.IMAGE_PER_ARTICLE,articleId:recordId})
