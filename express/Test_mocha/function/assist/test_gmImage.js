...
describe('getImageProperty_async', function() {
    ...
    it('size', async ()=>{
        ...
        assert.deepStrictEqual(result.width,1152)
        assert.deepStrictEqual(result.height,648)
    })
    ...
    it('res', async ()=>{
        ...
        console.log(`${JSON.stringify(result)}`)
        assert.deepStrictEqual(result,'37.79x37.79 pixels/centimeter')
        // assert.deepStrictEqual(result.msg.height,648)
    })
    it('filesize', async ()=>{
        let result=await gmImage.getImageProperty_async(gmInst,e_gmGetter.FILE_SIZE)
        // console.log(`${JSON.stringify(result)}`)
        assert.deepStrictEqual(result, sizeNum,5.1)
        assert.deepStrictEqual(result, sizeUnit,'Ki')
        // assert.deepStrictEqual(result.msg.height,648)
    })
