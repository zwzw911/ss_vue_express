...
let publicGroupId1CryptedByUser1,publicGroupId1CryptedByUser2,publicGroupId1CryptedByAdminRoot

let normalRecord={
    // [e_field.IMPEACH_COMMENT.]:'new impeach',
    [e_field.JOIN_PUBLIC_GROUP_REQUEST.PUBLIC_GROUP_ID]:undefined,
...
describe('user1 register unique check:',async  function() {
    ...
    before('prepare', async function () {
        ...
        let publicGroupInfo={
            [e_field.PUBLIC_GROUP.NAME]:'test group',
            [e_field.PUBLIC_GROUP.JOIN_IN_RULE]:e_publicGroupJoinInRule.PERMIT_ALLOW
        }
        data.values={}
    ...
    /***************    create  join public group request ***************/
    describe('create join public group request:',async  function() {
        let sess
        before('prepare', async function () {
            url=''
        ...
        it('1.1 user not login', async function() {
            // ap.inf('sess',sess)
            expectedErrorRc=controllerError.dispatch.post.notLoginCantCreateJoinRequest.rc
        ...
        it('1.2 user in penalize cant create join public group request ', async function() {
            expectedErrorRc=controllerError.dispatch.post.userInPenalizeCantCreateJoinRequest.rc
        ...
        it('1.3 inputValue recordInfo: , fieldValue to be decrypt not string', async function() {
            ...
            data={values:{[e_part.RECORD_INFO]:{[e_field.JOIN_PUBLIC_GROUP_REQUEST.HANDLE_RESULT]:1234}}}
        ...
        /***************    update join public group request ***************/
        describe('update join public group update',async  function() {
            ...
            it('2.1 user not login', async function() {
                // ap.inf('sess',sess)
                expectedErrorRc=controllerError.dispatch.put.notLoginCantUpdateJoinRequest.rc
            ...
            it('2.2 user in penalize cant update public group', async function() {
                expectedErrorRc=controllerError.dispatch.put.userInPenalizeCantUpdateJoinRequest.rc
