...
/*        adminRootSess = await adminUserComponentFunction.adminUserLogin_returnSess_async({
    ...
    /***************    create  new article   ***************/
    describe('create comment:',async  function() {
        let sess
        before('prepare', async function () {
            url=''
        ...
        it('1.0 unmatch url', async function() {
            ...
            await misc_helper.postDataToAPI_compareCommonRc_async({APIUrl:'/article_comment/test',sess:sess,data:data,expectedErrorRc:expectedErrorRc,app:app})
        });
        it('1.1 user not login', async function() {
            // ap.inf('sess',sess)
            expectedErrorRc=controllerError.dispatch.post.notLoginCantCreateComment.rc
        ...
        it('1.3 crypted objectId format wrong', async function() {
            normalRecord[e_field.ARTICLE_COMMENT.ARTICLE_ID]='1'
