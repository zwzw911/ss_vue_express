...
* */
async function createSendRecommend_returnCryptedId_async({data,userSess,app}) {
    // console.log(`createImpeach_async===>data.values ===>${JSON.stringify(data.values)}`)
    return new Promise(function(resolve,reject){
        request(app).post('/recommend').set('Accept', 'application/json').set('Cookie', [userSess]).send(data)
...
async function updateReceivedUnreadCommendToRead_async({data,userSess,app}) {
    // console.log(`createImpeach_async===>data.values ===>${JSON.stringify(data.values)}`)
    return new Promise(function(resolve,reject){
        request(app).put('/recommend/readUnreadRecommend').set('Accept', 'application/json').set('Cookie', [userSess]).send(data)
            .end(function (err, res) {
                // if (err) return done(err);
                // console.log(`res ios ${JSON.stringify(res)}`)
                let parsedRes = JSON.parse(res.text)
...
module.exports={
    createSendRecommend_returnCryptedId_async,
    updateReceivedUnreadCommendToRead_async,
