...
async function ruleCheckAll_async({parameter,expectedRuleToBeCheck,expectedFieldName}){
    ...
    if(method===e_method.DELETE){
        // it(`DELETE: recordId type check`,async function(){
        console.log(`==========================================================================`)
        console.log(`===================     DELETE: RECORD_ID dataType   =====================`)
        console.log(`==========================================================================`)
        let testDataFormat=generateTestData_API.generateTestDataForRecordIdDataType(parameter)
        await ruleTest_DELETE_async(parameter,testDataFormat,validateFormatError.inputValuePartRecordIdValueFormatWrong.rc)
        // })
        // it(`DELETE: recordId value check`,async function(){
        console.log(`==========================================================================`)
        console.log(`===================     DELETE: RECORD_ID value   ========================`)
        console.log(`==========================================================================`)
    ...
    if(method===e_method.CREATE || method===e_method.UPDATE){
        // console.log(`method is create/update===>`)
        //coll级别
        // it(`misc test `, async function(){
            console.log(`==========================================================================`)
            console.log(`======================     CU: misc test start   =========================`)
            console.log(`==========================================================================`)
            let testDataMisc=generateTestData_API.generateMiscTestData(parameter)
            await ruleTest_CU_async(parameter,testDataMisc,validateFormatError.recordInfoFiledRuleNotDefine.rc,RC_LEVEL.COMMON)
            console.log(`==========================================================================`)
        ...
        for(let singleFieldName in collRule){
            ...
            // it(`${[singleFieldName]}: ${`undefined`} `, async function(){
                ...
                generatedTestData=generateTestData_API.generateTestDataForRecordInfoDataType(parameter)
                await ruleTest_CU_async(parameter,generatedTestData,validateValueError.CUDTypeWrong.rc,RC_LEVEL.FIELD)
                console.log(`==========================================================================`)
            ...
            for(let singleRuleName in collRule[singleFieldName]){
                ...
                if(-1!==allValidRuleName.indexOf(singleRuleName)){
                    ...
                    // it(`${[singleFieldName]}: ${singleRuleName} `, async function(){
                        ...
                        //field中每个rule级别
                        generatedTestData=generateTestData_API.generateTestDataForRecordInfoValue(parameter)
                        await ruleTest_CU_async(parameter,generatedTestData,collRule[singleFieldName][singleRuleName]['error']['rc'],RC_LEVEL.FIELD)
                        console.log(`==========================================================================`)
...
* */
async function ruleTest_CU_async(parameter,generatedTestData,expectedErrorRc,rcLevel=RC_LEVEL.FIELD){
    let {sess,APIUrl,normalRecordInfo,method=e_method.CREATE,fieldName,singleRuleName,collRule,collName,app}=parameter
    let data={values:{}}
...
async function ruleTest_DELETE_async(parameter,generatedTestData,expectedErrorRc){
