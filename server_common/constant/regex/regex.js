...
const regex={
    ...
    sessionId:/\w+/,
    ipv4:/^(((\d{1,2})|(1\d{2})|(2[0-4]\d)|(25[0-5]))\.){3}((\d{1,2})|(1\d{2})|(2[0-4]\d)|(25[0-5]))$/, //ipv4
    sha1Hash:/^[0-9a-f]{40}$/,
    sha256:/^[0-9a-f]{64}$/,
    sha512:/^[0-9a-f]{128}$/,
...
/*    ^(?![a-zA-z]+$)==>匹配不包含纯字母的字符，需要前向，否则后向会使用[a-zA-Z\d!@#$%^&*]
    前面3个前向排除纯字母，纯数字，和纯特殊符号，最有一个匹配字母、数字、特殊符号*/
    ...
    lua:{
        ARGVConvert:/([{,])"(\w+)"/g, //传入lua脚本的参数固定为一个JSON转换的字符；因为lua无法处理引号(双或者单)括起的key，所以需要通过正则替换。对象===>JSON.stringify()===>字符===>正则去除双引号（stringify都为双引号）转换后的字符
    },
    ...
    clientRemoveDoubleQuotes:/("pattern":)\s("\/)(.+?)(\/")/g,  //    '$1/$3/,"'
