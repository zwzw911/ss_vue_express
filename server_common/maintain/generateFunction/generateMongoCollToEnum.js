...
function getFileNameInFolder(dirPath,skipFilesArray){
    let indent=`\ \ \ \ `
    let convertedEnum=''
    ...
    if(isDir){
        ...
        for(let singleFileDir of dirContent){
            ...
            if(isDir){
                ...
                convertedEnum+=getFileNameInFolder(tmpFileDir,skipFilesArray)
            }
            if(isFile){
...
function writeResult(toBeReadDir,resultWriteFilePath,skipFilesArray){
    ...
    convertedEnum+=getFileNameInFolder(toBeReadDir,skipFilesArray)
    convertedEnum+=`}\r\n`
...
writeResult('../model/mongo/structure','../constant/enum/DB_Coll.js',skipFilesArray)*/
