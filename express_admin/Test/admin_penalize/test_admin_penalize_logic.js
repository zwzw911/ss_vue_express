...
describe('create penalize', async function() {
    ...
    before('prepare===>create penalize', async function(){
        ...
        await API_helper.createAdminUser_async({sess:rootSess,userData:Object.assign({},testData.admin_user.adminUser1, {[e_field.ADMIN_USER.USER_PRIORITY]:[e_adminPriorityType.PENALIZE_USER]}),adminApp:adminApp})
        ...
        await API_helper.createAdminUser_async({sess:rootSess,userData:Object.assign({},testData.admin_user.adminUser2, {[e_field.ADMIN_USER.USER_PRIORITY]:[e_adminPriorityType.REVOKE_PENALIZE]}),adminApp:adminApp})
        ...
        user1Id=await test_helper.getUserId_async({userAccount:testData.user.user1.account})

        /*              delete user1 penalize record            */
        await test_helper.deleteUserPenalize_async({account:testData.user.user1.account})
...
describe('update user error:', function() {
    ...
    before('prepare===>update user error', async function(){
        ...
        await test_helper.deleteAdminUserAndRelatedInfo_async(testData.admin_user.user2.name)
